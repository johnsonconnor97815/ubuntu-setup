...
do_install() {
  ...
  log_info "Installed fcitx5 + RIME. Log out and back in so the input-method environment"
  log_info "takes effect, then switch input methods with Ctrl+Space (default)."
...
do_remove() {
  ...
  log_info "and your RIME data so other engines / a re-install keep working."
...
ui() {
  ...
  while true; do
    ...
    case "${dkind[$sel]}" in note|spacer|header|envrow)
      for (( g=0; g<n; g++ )); do sel=$(( (sel+1)%n )); case "${dkind[$sel]}" in note|spacer|header|envrow) ;; *) break ;; esac; done ;;
    esac
    ...
    for (( i=0; i<n; i++ )); do
      case "${dkind[$i]}" in
        ...
        envrow) ui_move "$row" 4; printf '\033[K%s' "${dlabel[$i]}" >&"$_UI_FD" ;;
    ...
    ui_read_key
    case "$UI_KEY" in
      up|k)   for (( g=0; g<n; g++ )); do sel=$(( (sel-1+n)%n )); case "${dkind[$sel]}" in note|spacer|header|envrow) ;; *) break ;; esac; done ;;
      down|j) for (( g=0; g<n; g++ )); do sel=$(( (sel+1)%n ));   case "${dkind[$sel]}" in note|spacer|header|envrow) ;; *) break ;; esac; done ;;
      space)
        case "${dkind[$sel]}" in
          schema)
            local id="${did[$sel]}"
