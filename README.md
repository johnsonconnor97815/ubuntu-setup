...
并行度、条目过滤、诊断产物等旋钮见 `tests/integration/__init__.py` 与各套件模块的 docstring。`UBUNTU_SETUP_VERIFY_BASE_IMAGE`(以字节一致的官方镜像源替换 `ubuntu:24.04`,如 `mirror.gcr.io/library/ubuntu:24.04`)仅供 docker.io 被本机代理劫持的开发机使用。
