...
- A **`PreconditionError`** — the entry isn't applicable on this host (e.g. `snapd` absent, or a `requires` capability missing) — is **skipped** (recorded `skipped`) and the run **continues**. This is detect-and-skip, not failure. (The earlier `software-properties-common` example is gone with the superseded `add-apt-repository` PPA design — the shipped `ppa` provider needs no such precondition; see [catalog-and-providers.md](./catalog-and-providers.md).)
