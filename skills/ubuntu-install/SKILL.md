...
- **② Per-command sudo, never whole-root** — escalate only the single command that needs root, via `sudo_run`. **Never `sudo npm install -g`** — `npm_global_writable` is the predicate, and npm-only installers call `npm_ensure_user_prefix`, which (no sudo) sets a user-writable prefix in `~/.npm-global` (a dedicated npm dir kept out of `~/.local/bin` so npm globals don't collide with native installers; writes `~/.npmrc`) when the prefix is a system default, or refuses a custom unwritable one. **Never `apt-key`** (use `add_apt_keyring` / `add_apt_source`). **Never write a NOPASSWD rule yourself.**
