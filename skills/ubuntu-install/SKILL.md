...
- The repo ships a **seed set** (`git`, `curl`, `zsh`, `docker`, `node`, `claude`, `codex`) plus the machinery. Coverage **grows** because you author new scripts and fix stale ones. That is the point: scripts give stable, testable execution; you keep them current and cover the long tail.
...
1. **Discover.** `swkit list` (every script grouped by category, with `[installed]` tags) or `swkit search <term>` (matches key/name/desc). To read details, run `scripts/<x>.sh meta` or open the script.
...
1. **Start from the template.** `cp $KIT_HOME/scripts/TEMPLATE.sh $KIT_HOME/scripts/<key>.sh`. Keep its skeleton: `#!/usr/bin/env bash`, `set -Eeuo pipefail`, locate and `source lib/common.sh`, define `meta` / `status` / `do_install` / `do_remove` (and `do_configure` only if there is a *safe, minimal* config step), `usage`, and **END with `kit_dispatch "$@"`**.
...
8. **Test, then commit.** `bash -n scripts/<key>.sh`; run the script's own `status` **before** and **after**; only then run it for real, with the user's confirmation. `swkit list` / the bootstrap TUI pick it up automatically once it is executable with valid `meta`. Changes are git-tracked under `$KIT_HOME`: `git commit` with a clear message, and consider a PR upstream.
