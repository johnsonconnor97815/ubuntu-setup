"""Catalog-wide container tier for the deb batch: really converge EVERY
entries-deb chain end-to-end in disposable Docker ``ubuntu:24.04`` guests.

One guest per product (12 repo-mode chains + 6 direct-mode entries — the 18
final-list deb products beyond the pilot; docker + vscode stay covered by
``test_container_deb``). Per chain the protocol mirrors the pilot:

- a ``requires: [desktop]`` product first proves the VISIBLE skip on the
  desktop-less guest, then the desktop marker is planted and the real install
  must behave like on a desktop machine;
- fresh dry-run predicts the change(s) and mutates nothing (no manifest, no
  sources file);
- the real install converges the whole chain in topological order (asserted
  ...
  repo entry), and costs EXACTLY one ``apt-get update`` for a repo chain /
  ZERO for a direct .deb (the freshness guard end-to-end — counted from the
  in-guest audit log);
- the landing probe really runs;
- the idempotent re-run is the pure ``check()`` no-op — including the repo
  entry (byte-identical files + the fetched-lists probe), with NO further
  ``apt-get update`` and a second all-``ok`` transaction.

Gated like the rest of the tier:

    UBUNTU_SETUP_INTEGRATION=1 python -m unittest \\
        tests.integration.test_container_deb_catalog -v

Knobs: ``UBUNTU_SETUP_INTEGRATION_PARALLEL`` (default 4),
``UBUNTU_SETUP_INTEGRATION_ENTRIES`` (comma-separated product filter),
``UBUNTU_SETUP_INTEGRATION_TIMEOUT`` (per-exec seconds; browser/vendor .debs
are 100-200MB downloads). A per-run report lands under the artifacts dir
(``catalog-deb-<UTC stamp>.log``).
"""
...
#: vendor .debs (chrome/edge/zoom/...) are 100-200MB; one exec holds a download
...
#: the chains the PILOT suite (test_container_deb) already covers
PILOT_COVERED = frozenset({
    "docker-repo", "docker", "docker-buildx", "docker-compose",
    "vscode-repo", "vscode",
})
...
class ChainSpec:
    """One product of the deb batch: its expected planner order and landing
    proof. ``repo_id`` is None for a direct-mode .deb (no repo entry, and the
    run must cost ZERO apt-get updates instead of exactly one)."""
...
#: product id -> chain spec. Hand-written (like PROBES in the apt suite) so the
...
CHAINS: "dict[str, ChainSpec]" = {
    # -- repo-mode chains -------------------------------------------------------
    "firefox": ChainSpec(
        chain_order=("curl", "gnupg", "firefox-repo", "firefox"),
        probe=("test", "-x", "/usr/bin/firefox"),
        repo_id="firefox-repo",
    ),
    "brave": ChainSpec(  # binary keyring: no gnupg in the chain
        chain_order=("curl", "brave-repo", "brave"),
        probe=("test", "-x", "/usr/bin/brave-browser"),
        repo_id="brave-repo",
    ),
    "edge": ChainSpec(
        chain_order=("curl", "gnupg", "edge-repo", "edge"),
        probe=("test", "-x", "/usr/bin/microsoft-edge-stable"),
        repo_id="edge-repo",
    ),
    "signal": ChainSpec(
        chain_order=("curl", "gnupg", "signal-repo", "signal"),
        probe=("test", "-x", "/opt/Signal/signal-desktop"),
        repo_id="signal-repo",
    ),
    "spotify": ChainSpec(
        chain_order=("curl", "gnupg", "spotify-repo", "spotify"),
        probe=("test", "-x", "/usr/share/spotify/spotify"),
        repo_id="spotify-repo",
    ),
    "mongodb": ChainSpec(  # mongod itself needs AVX/systemd: existence probe
        chain_order=("curl", "gnupg", "mongodb-repo", "mongodb"),
        probe=("test", "-x", "/usr/bin/mongod"),
        repo_id="mongodb-repo",
    ),
    "redis": ChainSpec(
        chain_order=("curl", "gnupg", "redis-repo", "redis"),
        probe=("redis-server", "--version"),
        repo_id="redis-repo",
    ),
    "vscodium": ChainSpec(
        chain_order=("curl", "gnupg", "vscodium-repo", "vscodium"),
        probe=("test", "-x", "/usr/bin/codium"),
        repo_id="vscodium-repo",
    ),
    "sublime-text": ChainSpec(  # flat repo (Suites: apt/stable/)
        chain_order=("curl", "gnupg", "sublime-text-repo", "sublime-text"),
        probe=("test", "-x", "/usr/bin/subl"),
        repo_id="sublime-text-repo",
    ),
    "kubectl": ChainSpec(  # flat repo (Suites: /)
        chain_order=("ca-certificates", "curl", "gnupg", "kubectl-repo",
                     "kubectl"),
        probe=("kubectl", "version", "--client"),
        repo_id="kubectl-repo",
    ),
    "terraform": ChainSpec(
        chain_order=("curl", "gnupg", "terraform-repo", "terraform"),
        probe=("terraform", "version"),
        repo_id="terraform-repo",
    ),
    "gh": ChainSpec(  # binary keyring: no gnupg in the chain
        chain_order=("curl", "gh-repo", "gh"),
        probe=("gh", "--version"),
        repo_id="gh-repo",
    ),
    # -- direct-mode .debs (no repo entry, zero apt-get updates) -----------------
    "chrome": ChainSpec(
        chain_order=("curl", "chrome"),
        probe=("test", "-x", "/usr/bin/google-chrome-stable"),
    ),
    "vivaldi": ChainSpec(
        chain_order=("curl", "vivaldi"),
        probe=("test", "-x", "/usr/bin/vivaldi-stable"),
    ),
    "discord": ChainSpec(  # the .deb ships /usr/bin/discord (launcher script)
        chain_order=("curl", "discord"),
        probe=("test", "-x", "/usr/bin/discord"),
    ),
    "zoom": ChainSpec(
        chain_order=("curl", "zoom"),
        probe=("test", "-x", "/opt/zoom/ZoomLauncher"),
    ),
    "obsidian": ChainSpec(
        chain_order=("curl", "obsidian"),
        probe=("test", "-x", "/opt/Obsidian/obsidian"),
    ),
    "steam": ChainSpec(  # launcher only; i386 multiarch stays a runtime note
        chain_order=("curl", "steam"),
        probe=("test", "-x", "/usr/bin/steam"),
    ),
}
...
def _selected_products() -> "list[str]":
    ...
    if unknown:
        raise AssertionError(
            f"UBUNTU_SETUP_INTEGRATION_ENTRIES names unknown deb products: "
            f"{unknown}"
        )
    return wanted
...
class TestCatalogDebEntries(unittest.TestCase):
    """The whole entries-deb batch, end-to-end: one fresh guest per product,
    ...
    def setUpClass(cls) -> None:
        ...
        # coverage sync: every deb-typed entry and every repo-backed apt
        # package must be covered by EXACTLY one of (pilot suite, this suite) —
        # adding a deb entry without deciding its chain fails loudly here, and
        # so does duplicating a pilot-covered chain in CHAINS
        deb_world = {e.id for e in catalog.values() if e.type == "deb"} | {
            e.id for e in catalog.values()
            if e.type == "apt"
            and any(catalog[dep].type == "deb" for dep in e.depends_on)
        }
        chain_ids = set()
        ...
        covered = chain_ids | PILOT_COVERED
        missing = sorted(deb_world - covered)
        stale = sorted(covered - deb_world)
        overlap = sorted(chain_ids & PILOT_COVERED)
        if missing or stale or overlap:
            raise AssertionError(
                f"CHAINS out of sync with the catalog deb batch: "
                f"missing={missing} stale={stale} "
                f"pilot-overlap={overlap}"
            )
    ...
    def _count_audit_updates(session: harness.GuestSession) -> int:
        ...
        guard's batch dedupe asserted end-to-end. Vendor-.deb postinst scripts
        run apt internally without appearing here, by design."""
    ...
    def _verify_chain(self, session: harness.GuestSession, entry_id: str,
                      spec: ChainSpec, sources_path: "str | None") -> None:
        """fresh dry-run -> real install (order + exact update count) ->
        idempotent re-run (all ok, no further update)."""
        user = session.profile.user
        py = session.profile.python
        expected_updates = 1 if spec.repo_id is not None else 0
        changed_ids = ((spec.repo_id, entry_id) if spec.repo_id is not None
                       else (entry_id,))
        ...
        if sources_path is not None:
            ...
            _ensure(not still_absent.ok,
                    "a dry run must not configure the repo")

        # -- 2. real install: chain order, outcomes, exact update count ----------
        inst = session.exec(
        ...
        _ensure(updates == expected_updates,
                f"the chain must cost exactly {expected_updates} apt-get "
                f"update(s) (freshness guard), saw {updates}")
        ...
        _ensure(self._count_audit_updates(session) == expected_updates,
    ...
    def _verify_one(self, entry_id: str, *, needs_desktop: bool,
                    sources_path: "str | None") -> float:
        spec = CHAINS[entry_id]
        started = time.monotonic()
        driver = DockerDriver(
            self.image,
            name=unique_name(f"usetup-debcat-{entry_id}"),
            run=_patient_run,
        )
    ...
    def _write_report(lines: "list[str]") -> Path:
        ...
        report = target / f"catalog-deb-{stamp}.log"
        report.write_text("\n".join(lines) + "\n", encoding="utf-8")
        return report

    # -- the acceptance test --------------------------------------------------------
    def test_install_every_deb_chain(self):
        """Every entries-deb product really converges on a clean 24.04 guest
        and re-runs idempotently; failures are aggregated (the run never stops
        at the first bad chain) and a duration report is kept under artifacts."""
        ...
        def sources_path_for(spec: ChainSpec) -> "str | None":
            if spec.repo_id is None:
                return None
            name = catalog[spec.repo_id].fields["name"]
            return f"/etc/apt/sources.list.d/{name}.sources"
        ...
        def work(entry_id: str) -> None:
            ...
            try:
                ...
                print(f"[deb-verify] FAIL {entry_id} "
                      f"({time.monotonic() - t0:.0f}s): {exc}", flush=True)
            else:
                outcomes[entry_id] = ("ok", duration)
                print(f"[deb-verify] ok   {entry_id} ({duration:.0f}s)",
                      flush=True)
        ...
        lines = [
            f"catalog deb batch verification — {len(self.products)} chains, "
        ...
        print(f"[deb-verify] report: {report}", flush=True)
