...
- `scripts/*.sh`——脚本集合,种子集:`git` `curl` `zsh` `docker` `node` `claude` `codex`(`docker`、`git`、`zsh` 另有 `configure`;**每个脚本都手写了自己的 `ui()`**);外加 `scripts/TEMPLATE.sh`(LLM 演进时 `cp` 起步,含 `ui()` 注释样例)。
